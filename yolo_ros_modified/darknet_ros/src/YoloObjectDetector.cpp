...
YoloObjectDetector::YoloObjectDetector(ros::NodeHandle nh)
  ...
  init();    //zhangjiadong  在构造函数中,初始化yolo
}
...
void YoloObjectDetector::init() {
  ...
  yoloThread_ = std::thread(&YoloObjectDetector::yolo, this);   //zhangjiadong 主函数的线程
...
void YoloObjectDetector::cameraCallback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  time_newImage = msg->header.stamp;
  try {
...
void* YoloObjectDetector::detectInThread() {
  ...
  if (count == 0) {   //zhangjiadong 这里代表检测到的物体为0
    roiBoxes_[0].num = 0;
  } else {
...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num >= 0 && num <= 100) {
      // 在rosBoxes_中存储,当前当前帧中的物体.rosBoxes_中的每一行(共numClasses_行),代表一类物体,列数代表同一类物体的数量.
      for (int i = 0; i < num; i++) {
      ...
      for (int i = 0; i < numClasses_; i++) {
        if (rosBoxCounter_[i] > 0) {
          ...
          for (int j = 0; j < rosBoxCounter_[i]; j++) {
            ...
            boundingBox.Class = classLabels_[i];  // string 名字
            boundingBox.id = i;   //label的 id数字
      ...
      boundingBoxesResults_.header.stamp = time_newImage;//ros::Time::now();
      boundingBoxesResults_.header.frame_id = "detection";
      boundingBoxesResults_.image_header = headerBuff_[(buffIndex_ + 1) % 3];
      boundingBoxesPublisher_.publish(boundingBoxesResults_);  //zhangjiadong 问题就在这,只有物体数量大于0,才publish.
  } else {
    darknet_ros_msgs::ObjectCount msg;
    msg.header.stamp = ros::Time::now();
    msg.header.frame_id = "detection";
